...
def _print_stats(label: str, trader: PaperTrader, sigs: int) -> None:
    s  = trader.get_stats()
...
def run_backtest(
    ...
    print(f"      有效模擬 bar：{span} 根（約 {span*4//24} 天）")
    ...
    print(f"      BTC 空頭環境：{bear_bars}/{span} 根（{bear_bars/span*100:.1f}%）")
    ...
    for i, sym in enumerate(symbols, 1):
        ...
        print(f"  [{i:3d}/{len(symbols)}] {sym:15s} {'OK  ' if ok else '跳過'}  {tag}")
        if ok:
            sym_data[sym] = {
                "c4h":       c4h,
                "c1h":       c1h,
    ...
    print("  回測結果比較")
    print("=" * 64)
    _print_stats("有 Regime Filter", trader_w,  sig_w)
    _print_stats("無 Regime Filter", trader_wo, sig_wo)
    ...
    if show_detail:
        ...
        for t in sorted(trader_w.trade_history, key=lambda x: x["open_ms"]):
            if not t["full_close"]:
                continue
            print(f"  {fmt_tw(t['open_ms'])}  {t['symbol']:15s} {t['direction']:5s} "
